...
// Reason: Upstash per-command request quota was being exhausted by this worker's
// blocking poll loop (bzpopmin) against bull:responses:marker.
if (!(process.env.BULLMQ_ENABLED === 'true' && process.env.REDIS_URL)) {
  console.warn('⚠️  BullMQ response worker is paused. (Set BULLMQ_ENABLED=true and REDIS_URL to re-enable.)');
  module.exports = null;
} else {
