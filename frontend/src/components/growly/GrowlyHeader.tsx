...
export function GrowlyHeader({ userName, targetGoal, onMenuClick, onProfileClick }: GrowlyHeaderProps) {
  ...
  return (
    <header className="glass-toolbar sticky top-0 z-20 flex h-[70px] items-center justify-between border-b px-5 sm:px-7 lg:px-9">
      <div className="flex min-w-0 items-center gap-3">
        <button onClick={onMenuClick} className="rounded-lg p-2 text-[#5E687D] hover:bg-white/60 hover:text-[#18213A] md:hidden" aria-label="Open navigation"><Menu size={20} /></button>
      ...
      <button onClick={onProfileClick} className="group flex items-center gap-2 rounded-xl px-2 py-1.5 text-left transition-colors hover:bg-white/60 focus:outline-none focus:ring-2 focus:ring-[#AABBEA]">
